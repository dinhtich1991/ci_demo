...
if ( ! function_exists('frontend_menu_left'))
{
	function frontend_menu_left($param =''){
	   $CI =& get_instance();
			   ...
			   if(!empty($valMain['url'])){
				  ...
				  // echo $valMain['module_id']; die;
				   if($valMain['module'] == 'article_category'){
					$str = $str.frontend_menu_left_article_category($param['article_category'], $valMain['module_id']);
				   }
			   }
		   }
		   $str = $str.'</ul>';
...
if ( ! function_exists('frontend_submenu'))
{
	function frontend_submenu($param =''){
	   ...
	   if(isset($menu) && count($menu)){
		   ...
		   foreach($menu as $keyMain => $valMain){
			   if(!empty($valMain['url'])){
				   $str = $str.'<li><a href="javascript:avoid()">'.$valMain['title'].'</a></li>';
			   }
			   else if(!empty($valMain['module']) && $valMain['module_id'] > 0){
				  // echo $valMain['module_id']; die;
				   if($valMain['module'] == 'article_category'){
	...
	function frontend_submenu_article_category($data, $id = 0){
		if(isset($data) && count($data)){
			...
			foreach($data as $keyMain => $valMain){
				if($valMain['id'] == $id){
					$_tempMain = $_tempMain.'<li><a href="javascript:avoid()" title="'.htmlspecialchars($valMain['title']).'">'.$valMain['title'].'</a>';
			...
			//return !empty($_tempItem)?'<ul>'.$_tempItem.'</ul>':'';
		}
	}
}

if ( ! function_exists('frontend_menu_left_article_category'))
{
	function frontend_menu_left_article_category($data, $id = 0){
		if(isset($data) && count($data)){
			// main
			$_tempMain = '';
			foreach($data as $keyMain => $valMain){
				if($valMain['id'] == $id){
					$_tempMain = $_tempMain.'<li><a href="javascript:avoid()" title="'.htmlspecialchars($valMain['title']).'" class="main">'.$valMain['title'].'</a>';
					
					// item
					$_tempItem = '';
					foreach($data as $keyItem => $valItem){
						if($valMain['id'] == $valItem['parentid']){
							$_tempItem = $_tempItem.'<li><a href="'.frontend_link_menu($valItem['route'], $valItem['alias'], $valItem['id'], '68').'" title="'.htmlspecialchars($valItem['title']).'" class="item">'.$valItem['title'].'</a>';
							//SUB
							$_tempSub = '';
							foreach($data as $keySub => $valSub){
								if($valItem['id'] == $valSub['parentid']){
									$_tempSub = $_tempSub.'<li><a href="'.frontend_link_menu($valSub['route'], $valSub['alias'], $valSub['id'], '68').'" title="'.htmlspecialchars($valSub['title']).'" class="sub">'.$valSub['title'].'</a>';
									// Cap 4
									$_tempChildren = '';
									foreach($data as $key => $val){
										if($valSub['id'] == $val['parentid']){
											$_tempChildren = $_tempChildren.'<li><a href="'.frontend_link_menu($val['route'], $val['alias'], $val['id'], '68').'" title="'.htmlspecialchars($val['title']).'" class="children">'.$val['title'].'</a>';
										}
										continue;
									}
									$_tempChildren = !empty($_tempChildren)?'<ul>'.$_tempChildren.'</ul>':'';
...
if ( ! function_exists('frontend_menu_left_getdata'))
{
	function frontend_menu_left_getdata($table = 'article_category'){
	   $CI =& get_instance();
	   $_lang = $CI->session->userdata('_lang');
