...
class Article extends MY_Controller {
	...
	public function category($catid = 0, $page = 1){
		...
		/*
		$child = $this->my_nestedset->recursive($catid, $this->my_nestedset->arr('article_category'));
		//$child = $this->db->select('id')->from('article_category')->where(array('parentid' => $catid))->get()->result_array();
		if(isset($child) && !empty($child)){
			foreach($child as $key => $val){
				if(isset($key) && !empty($key)){
					$product[] = $this->db->from('article_item')->where(array('parentid' => $val['id']))->get()->result_array();
				}
			}
		}
		else{
			$product[] = $this->db->from('article_item')->where(array('parentid' => $catid))->get()->result_array();
		}
		*/
		//print_r($category); die;
		//$product = $this->db->from('acticle_item')->where(array('parentid' => $catid))->get()->result_array();
		//print_r($product); die;
		$config = $this->my_frontend->pagination_category();
		...
		if($config['total_rows'] > 0){
			...
			if($category['rgt'] - $category['rgt'] == 1){
				$data['data']['_list'] = $this->db->select('id, title, total, hot, sale, price, price_sale, alias, route, parentid, image, description, created')->from('article_item')->where(array('lang' => $_lang, 'parentid' =>$catid, 'publish' => 1))->limit($config['per_page'],($page -1) * $config['per_page'])->order_by('order desc, id desc')->get()->result_array();	
			}
			else{
				$data['data']['_list'] = $this->db->select('id, title, total, hot, sale, price, price_sale, alias, route, parentid, image, description, created')->from('article_item')->where(array('lang' => $_lang, 'publish' => 1))->where_in('parentid', $_children)->limit($config['per_page'],($page -1) * $config['per_page'])->order_by('order desc, id desc')->get()->result_array();
			}
