...
class My_nestedset_galary {
	...
	public function check_empty($table = ''){
		...
		if($count == 0){
			...
			die($this->CI->my_string->php_redirect(BASE_URL.'backend/'.current(explode('_',$table)).'/add'.end(explode('_',$table))));
		}
	}
	
	//Mảng lựa chọn sử dụng cho danh mục đổ xuống
	
	public function dropdown($table = '', $param = NULL, $type = 'bst'){
		$temp = null;
	...
	public function level($table = ''){
		$data = $this->CI->my_nestedset_galary->recursive(0, $this->CI->my_nestedset_galary->arr($table));
		if(isset($data) && count($data)){
			...
			foreach($data as $key => $val){
				...
				if($val['parentid'] == 0){
					...
					$count = $this->CI->my_nestedset_galary->get($table, array('id' => $val['parentid']));
	...
	public function lftrgt($table =''){
		...
		if(isset($data) && count($data)){
			...
			foreach($data as $key => $val){
				...
				$countSubbstdetail = count($this->recursive($val['id'], $data));
				// Các node đầu tiên trong level
				if(!isset($max[$val['level']])){
					$left = $i;
					$right = ($countSubbstdetail *2) +1 + $i;
					$max[$val['level']] = $right;
					if($left + 1 == $right){
						$flag = 1;
					}
					...
					//Các node được duyệt ngay sau lá
					if($flag = 1){
						$flag =0;
						$i = $max[$val['level']] + 1;
						$left = $i;
	...
	public function set($table =''){
		$this->CI->my_nestedset_galary->level($table);
		$this->CI->my_nestedset_galary->lftrgt($table);
	}
	
	public function check_parentid($table = '', $parentid = 0, $catid = 0){
		$_lang = $this->CI->session->userdata('_lang');
		if($parentid == $catid){
			$this->CI->form_validation->set_message('check_parentid', 'Không thể chọn chính nó làm danh mục cha.');
			return FALSE;
		}
		$data = $this->CI->db->select('lft, rgt')->from($table)->where(array('lang' => $_lang))->where(array('id' => $catid))->get()->row_array();
		if(isset($data) && count($data)){
			$chidren = $this->CI->db->select('id')->from($table)->where(array('lang' => $_lang))->where(array('lft >' => $data['lft'],'lft <' => $data['rgt']))->get()->row_array();
			
			if(isset($chidren) && count($chidren))
			{
				$tem[] = $chidren;
				foreach($tem as $key => $val){
					//if(!is_array($val)){is_array($val);var_dump($val); } die;
					if(isset($val['id']) && $parentid == $val['id']){
						$this->CI->form_validation->set_message('check_parentid', 'Không thể chọn thư mục con làm thư mục cha.');
						return FALSE;
					}
				}
			}
